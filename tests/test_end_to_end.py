...
"""
    ...
    with tempfile.NamedTemporaryFile(mode='w', suffix='.md', delete=False) as kb_file:
        kb_file.write(test_kb_content)
    ...
    try:
        ...
        # Use answer_once to test the full pipeline (this would normally be tested in isolation)
        ...
        context_block, packed_ids, used_tokens = pack_snippets(
            chunks, mmr_selected, pack_top=3
        )
        ...
        for file_key in ['chunks', 'emb', 'meta', 'bm25', 'index_meta', 'faiss_index']:
