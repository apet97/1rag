...
class TestIngestEndpoint:
    ...
    def test_ingest_accepts_empty_body(self, client):
        ...
        with patch("clockify_rag.api.resolve_corpus_path") as mock_resolve:
            mock_resolve.return_value = ("knowledge_base", True, ["knowledge_base"])
            response = client.post("/v1/ingest", json={})

        # Should return 200 (processing started) or 404 (file not found)
        assert response.status_code in [200, 404]

    def test_ingest_404_when_file_not_found(self, client):
        """Ingest should return 404 when input file not found."""
        with patch("clockify_rag.api.resolve_corpus_path") as mock_resolve:
            mock_resolve.return_value = (None, False, ["nonexistent.md"])
