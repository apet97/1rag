...
for QUERY in "${QUERIES[@]}"; do
    ...
    python3 clockify_support_cli_final.py ask "$QUERY" > /tmp/query_result.txt 2>&1 || echo "Query failed" > /tmp/query_result.txt
    QUERY_END=$(date +%s%3N)
...
if command -v ps &> /dev/null; then
    ...
    python3 clockify_support_cli_final.py ask "Test query" > /dev/null 2>&1 &
    PID=$!
    sleep 2
