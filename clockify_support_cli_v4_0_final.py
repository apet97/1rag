...
def _mount_retries(sess, retries: int):
    """Mount or update HTTP retry adapters. Supports urllib3 v1 and v2 - Task G.
    ...
    - POST/GET method retries at adapter level
    """
    ...
    # Build retry configuration - Task G: allow POST/GET
    retry_kwargs = {
        "total": retries,
        "connect": retries,
        "read": retries,
    ...
    # Detect urllib3 version and set method whitelist - Task G
    try:
    ...
    try:
        retry_strategy = Retry(**retry_kwargs, allowed_methods=frozenset(["POST", "GET"]), respect_retry_after_header=True)
    except TypeError:
        try:
            retry_strategy = Retry(**retry_kwargs, method_whitelist=frozenset(["POST", "GET"]))
        except TypeError:
...
def pack_snippets(chunks, order, pack_top=6, budget_tokens=CTX_TOKEN_BUDGET, num_ctx=DEFAULT_NUM_CTX):
    """Pack snippets respecting token budget AND hard snippet cap - Task C.

    Guarantees:
    - Never exceeds CTX_TOKEN_BUDGET
    - First item always included (truncated if needed with [TRUNCATED])
    ...
    for idx_pos, idx in enumerate(order):
        ...
        # For first item: always include (Task C: truncate if needed)
        if (idx_pos == 0 or not ids) and not top1_included:
            if t_est > CTX_TOKEN_BUDGET:
                # Task C: truncate first item to fit budget
                txt = truncate_to_token_budget(txt, CTX_TOKEN_BUDGET)
                t_est = CTX_TOKEN_BUDGET
        ...
        elif used + t_est <= CTX_TOKEN_BUDGET:
            ids.append(cid)
            seen_ids.add(cid)
            out.append(_fmt_snippet_header(c) + "\n" + txt)
            used += t_est

    # Task C: Add [TRUNCATED] marker to header if first item was truncated
    if first_truncated and out:
        out[0] = out[0].replace("]", " [TRUNCATED]]", 1)

    # Task C: return used_tokens
    return "\n\n---\n\n".join(out), ids, used
...
def answer_once(
    ...
    try:
        ...
        if debug:
            ...
            # Task B: include rerank_applied and rerank_reason in debug JSON
            debug_info = {
                "snippets": diag,
