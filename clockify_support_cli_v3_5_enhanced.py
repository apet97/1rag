    ...
    while True:
        try:
            ...
            try:
                ...
                # If stale or dead owner, try to remove and retry (enhanced logging)
                if is_stale or not pid_alive:
...
def _log_config_summary(use_rerank=False, pack_top=DEFAULT_PACK_TOP):
    """Log configuration summary at startup - Edit 13 (enhanced)."""
    proxy_trust = int(os.getenv("USE_PROXY", "0"))
    # Effective pack cap: enforced at len(ids) >= pack_top in pack_snippets()
    logger.info(
        "cfg ollama_url=%s gen_model=%s emb_model=%s retries=%d proxy_trust_env=%d "
        "timeouts.emb=(%d,%d) timeouts.chat=(%d,%d) headroom=%.2f threshold=%.2f pack_cap=%d rerank=%s",
        ...
        HEADROOM_FACTOR, DEFAULT_THRESHOLD, pack_top, "enabled" if use_rerank else "disabled"
    )
...
def answer_once(
    ...
    try:
        ...
        # Edit 12: one-line turn logging with proof-of-path (enhanced for verification)
        pack_ids_str = ",".join(ids[:5]) + ("..." if len(ids) > 5 else "")  # First 5 IDs
        logger.info(
            "turn model=%s seed=%s topk=%s pack=%s threshold=%s rerank=%s mmr_applied=true "
            "rerank_applied=%s selected=%d pack_ids=[%s] latency.total=%.1fs",
            GEN_MODEL, seed, top_k, pack_top, threshold, use_rerank,
...
# ====== SELF-CHECK TESTS (Edit 15, enhanced for verification) ======
def test_mmr_behavior_ok():
    ...
    # Verify MMR logic exists in answer_once (inlined at lines 1214-1233)
...
def test_pack_cap_enforced():
    """Verify len(ids) <= pack_top hard cap - Enhanced."""
    ...
    order = list(range(20))
    block, ids = pack_snippets(chunks, order, pack_top=6, budget_tokens=10000, num_ctx=8192)
...
def test_post_retry_logic():
    """Verify POST retry logic exists - Enhanced."""
...
def test_rerank_applied_when_enabled():
    """Verify reranker is called and influences order - Enhanced."""
    import inspect
    source = inspect.getsource(answer_once)
...
def run_selftest():
    """Run all self-check tests - Edit 15 (enhanced for verification)."""
    tests = [
...
def chat_repl(top_k=12, pack_top=6, threshold=0.30, use_rerank=False, debug=False, seed=DEFAULT_SEED, num_ctx=DEFAULT_NUM_CTX, num_predict=DEFAULT_NUM_PREDICT, retries=0):
    ...
    # Edit 13: log config summary at startup (now includes pack_cap and rerank status)
    _log_config_summary(use_rerank=use_rerank, pack_top=pack_top)
